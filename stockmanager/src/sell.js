...
import Main from "./main";
	...
	// 	if(this.state.taskList[i].price!='')
	// 	{
	...
	// }
	 ...
	 totalamount = (e)=>{
		...
		for(var i=0;i<this.state.taskList.length;i++)
		{
		   ...
		   {
			//    NotificationManager.info(`${this.state.taskList[i].product_name}=${total_amount}`);
			   if(i==this.state.taskList.length-1){
			   // NotificationManager.info(`${this.state.taskList[i].product_name}=${total_amount}`);
		   NotificationManager.warning(`Total Amount=${total}`,'', 2500);
		   }
...
: <button className="binput" onClick={(() =>this.clickOnDelete(val))} >-<i className="button" aria-hidden="true"></i></button>
	...
	// 	  <div style={{ marginTop: 20 }}>
	// 	  <label className="sfont">CUSTOMER NAME</label><br/>
	// 	  <input type="text" value={this.state.cname} onChange={this.handleChange} name="cname" className="sinput" required/>
	// 		 <hr/>
	// 	  <table className="">
	// 	  <thead>
	// 	  <tr className="sfont">
